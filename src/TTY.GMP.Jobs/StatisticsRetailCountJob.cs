...
namespace TTY.GMP.Jobs
{
    ...
    /// 定时统计销售单数Job
    /// </summary>
    public class StatisticsRetailCountJob : IJob
    {
        /// <summary>
        /// 开始统计时间
        /// </summary>
        ...
        {
            Log.Info("定时统计销售单数Job》》开始执行", this.GetType());
            Initialize();
            try
            {
                await Process();
                Log.Info("定时统计销售单数Job》》执行完成", this.GetType());
                Console.WriteLine($"{DateTime.Now}定时统计销售单数Job》》执行完成");
            }
            catch (Exception ex)
            {
        ...
        {
            var maxTime = (await _reportBll.GetStatisticsRetailCountMaxTime()).Date;
        ...
        {
            var retailBillCount = await _statisticsBillCountBll.GetAllRetailBillCount(_startTime.ToTimestamp(), _endTime.ToTimestamp());
            ...
            {
                _reportBll.AddStatisticsRetailCount(new StatisticsRetailCount()
                {
