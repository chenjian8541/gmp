...
namespace TTY.GMP.Jobs
{
    ...
    /// 零售商品统计
    /// </summary>
    public class StatisticsRetailGoodsJob : IJob
    {
        /// <summary>
        /// 开始统计时间
        /// </summary>
        ...
        {
            Log.Info("定时零售商品统计Job》》开始执行", this.GetType());
            Initialize();
            try
            {
                await Process();
                Log.Info("定时零售商品统计Job》》执行完成", this.GetType());
                Console.WriteLine($"{DateTime.Now}定时零售商品统计Job》》执行完成");
            }
            catch (Exception ex)
            {
        ...
        {
            var maxTime = (await _reportBll.GetStatisticsRetailGoodsMaxTime()).Date;
        ...
        private async Task Execution()
        {
            var stockInOutRecord = await _stockInOutRecordBll.GetStockInOut(_startTime.ToTimestamp(), _endTime.ToTimestamp(),
                $"{(int)BillTypeEnum.Retail},{(int)BillTypeEnum.RetailBack}");
            ...
            foreach (var record in stockInOutRecord)
            {
                ...
                var totalCount = -1 * record.TotalCount;
                var totalWeight = goodsSpecAnalysis * totalCount;
                var totalContentsWeight = totalWeight * goodsContentsAnalysis;
                _reportBll.AddStatisticsRetailGoods(new Entity.Database.StatisticsRetailGoods()
                {
